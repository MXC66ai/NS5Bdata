...
for i in range(1,164):
    for j in range(1,10):
        ...
        try:
          ...
          #cmd.load('./{i}/CHEMBL1145278_{i}_{j}.pdb', 'ligand') # file
          #cmd.save('./{i}/lkh-5278_{i}_{j}.pdb')
          interactions_by_site = retrieve_plip_interactions(f'./{i}/lkh-5278_{i}_{j}.pdb')
        
          selected_site = list(interactions_by_site.keys())[index_of_selected_site]
        
          with open("../saltbridge.csv",mode='a',newline='') as cf:
              print(f"$CHEMBL1145278_{i}_{j}\t"
