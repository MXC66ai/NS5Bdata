...
for i in range(1,1518):
    for j in range(1,10):
        try:
            ...
            cmd.load('1YVF_noligand.pdb', 'proteinA') # file
            cmd.load(f'CHEMBL1137317_{i}_{j}.pdb', 'ligand') # file
            cmd.save(f'1YVF_CHEMBL1137317_{i}_{j}.pdb')
            interactions_by_site = retrieve_plip_interactions(f'1YVF_CHEMBL1137317_{i}_{j}.pdb')
            ...
            if(len(dfs) !=0):
                with open("saltbridge.csv",mode='a',newline='') as cf:
                    print(f"$1YVF_CHEMBL1137317_{i}_{j}\t"
                    f"$selected {selected_site}",file=cf)
                    cf.close()
                dfs.to_csv('saltbridge.csv', mode='a', index=False, header=False)
            
            dfha = create_df_from_binding_site(interactions_by_site[selected_site], interaction_type="halogen")
            if(len(dfha) !=0):
                with open("halogen.csv",mode='a',newline='') as cf:
                    print(f"$1YVF_CHEMBL1137317_{i}_{j}\t"
                    f"$selected {selected_site}",file=cf)
                    cf.close()
                dfha.to_csv('halogen.csv', mode='a', index=False, header=False)
        
            dfhy = create_df_from_binding_site(interactions_by_site[selected_site], interaction_type="hydrophobic")
            if(len(dfhy) !=0):
                with open("hydrophobic.csv",mode='a',newline='') as cf:
                    print(
                    f"$1YVF_CHEMBL1137317_{i}_{j}\t"
                    f"$selected {selected_site}",file=cf)
                    cf.close()
                dfhy.to_csv('hydrophobic.csv', mode='a', index=False, header=False)
            ...
            if(len(dfh) !=0):
                with open("hbond.csv",mode='a',newline='') as cf:
                    ...
                    f"$selected {selected_site}",file=cf)
                    cf.close()
                dfh.to_csv('hbond.csv', mode='a', index=False, header=False)
            ...
            if(len(dfpi) !=0):
                with open("pistacking.csv",mode='a',newline='') as cf:
                    ...
                    f"$selected {selected_site}",file=cf)
                    cf.close()
                dfpi.to_csv('pistacking.csv', mode='a', index=False, header=False)
        except:
            print(f"1YVF_CHEMBL1137317_{i}_{j}")
