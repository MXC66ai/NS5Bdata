...
for i in range(2,127):
    for j in range(1,10):
        try:
            ...
            #cmd.load('/home/mxc/palm2/4ke5_nolig.pdb', 'proteinA') # file
            #cmd.load(f'palm2_4ke5_{i}_{j}.pdb', 'ligand') # file
            #cmd.save(f'4ke5_{i}_{j}.pdb')
            interactions_by_site = retrieve_plip_interactions(f'4ke5_{i}_{j}.pdb')
           
            selected_site = list(interactions_by_site.keys())[index_of_selected_site]
            dfs = create_df_from_binding_site(interactions_by_site[selected_site], interaction_type="saltbridge")
            if(len(dfs) !=0):
                with open("4ke5_saltbridge1.csv",mode='a',newline='') as cf:
                    print(f"$4ke5_{i}_{j}\t"
                    f"$selected {selected_site}",file=cf)
                    cf.close()
                dfs.to_csv('4ke5_saltbridge1.csv', mode='a', index=False, header=False)
            
            
                
            dfh = create_df_from_binding_site(interactions_by_site[selected_site], interaction_type="hbond")
            if(len(dfh) !=0):
                with open("4ke5_hbond1.csv",mode='a',newline='') as cf:
                    print(
                    f"$4ke5_{i}_{j}\t"
                    f"$selected {selected_site}",file=cf)
                    cf.close()
                dfh.to_csv('4ke5_hbond1.csv', mode='a', index=False, header=False)
            
            dfpi = create_df_from_binding_site(interactions_by_site[selected_site], interaction_type="pistacking")
            if(len(dfpi) !=0):
                with open("4ke5_pistacking1.csv",mode='a',newline='') as cf:
                    print(
                    f"$4ke5_{i}_{j}\t"
                    f"$selected {selected_site}",file=cf)
                    cf.close()
                dfpi.to_csv('4ke5_pistacking1.csv', mode='a', index=False, header=False)
        except:
            print(f"4ke5_{i}_{j}")
